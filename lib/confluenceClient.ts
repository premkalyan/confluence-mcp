...
export class ConfluenceClient {
  ...
  async uploadDocument(pageId: string, file: { name: string; data: Buffer; mimeType: string }, comment?: string) {
  ...
  async embedExistingAttachment(pageId: string, attachmentId: string, attachmentName: string) {
    const page = await this.getContentById(pageId, ['body.storage', 'version']);
    const currentContent = (page.body?.storage?.value as string) || '';
    const version = (page.version?.number as number) || 1;

    const embedHtml = `<ac:image><ri:attachment ri:filename="${attachmentName}"/></ac:image>`;
    const newContent = currentContent + embedHtml;

    return this.updatePage(pageId, page.title as string, newContent, version);
  }

  async uploadAndEmbedDocument(pageId: string, file: { name: string; data: Buffer; mimeType: string }, comment?: string) {
    const uploadResult = await this.uploadDocument(pageId, file, comment);
    const attachmentId = uploadResult.results?.[0]?.id;

    if (attachmentId) {
      return this.embedExistingAttachment(pageId, attachmentId, file.name);
    }

    return uploadResult;
  }

  async uploadAndEmbedAttachment(pageId: string, file: { name: string; data: Buffer; mimeType: string }, comment?: string) {
    return this.uploadAndEmbedDocument(pageId, file, comment);
  }
}
